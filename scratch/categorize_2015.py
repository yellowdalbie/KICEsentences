...
sol_dir = "Sol/2015"
ref_dir = "MD_Ref/2015"
excluded_dir = "Sol_Excluded/2015"
os.makedirs(excluded_dir, exist_ok=True)

# List all filenames from Sol and Excluded
all_filenames = set()
if os.path.exists(sol_dir):
    all_filenames.update(os.listdir(sol_dir))
if os.path.exists(excluded_dir):
    all_filenames.update(os.listdir(excluded_dir))
...
for f in sorted(list(all_filenames)):
    ...
    if "EXCLUDE" in status:
        ...
        keep_count += 1
        # It's a KEEP. Move from Excluded to Sol if needed
        excl_p = os.path.join(excluded_dir, f)
        sol_p = os.path.join(sol_dir, f)
        if os.path.exists(excl_p):
            # Only move if Sol doesn't exist or is a caution block
...
print(f"Done 2015. KEEP: {keep_count}, EXCLUDE: {exclude_count}")
